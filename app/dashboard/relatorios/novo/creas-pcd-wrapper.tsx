...
export function CreasPcdReportWrapper({
    ...
    useEffect(() => {
        ...
        async function loadData() {
            setLoading(true)
            try {
                const data = await getCreasPcdReport(directorateId, Number(month), Number(year))
                if (isMounted) {
                    setInitialData(data)
                    setAlreadySubmitted(!!data.id && data.status !== 'draft')
                }
            } catch (err) {
                console.error("Error loading CREAS PCD data", err)
            } finally {
                    ...
                    <div className="space-y-4">
                        <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] font-bold text-purple-600 dark:text-purple-400 uppercase tracking-widest">CREAS Pessoa Com Deficiência</span>
                            </div>
                        </div>
