...
import { Loader2, Save, ArrowLeft, FileText, Loader } from "lucide-react"
...
export function DailyFormClient({ directorateId }: { directorateId: string }) {
    ...
    // Fetch directorate info and existing report
    useEffect(() => {
        async function loadData() {
            setFetching(true)

            // 1. Fetch Directorate Name
            const { data: dir } = await supabase
                .from('directorates')
                .select('name')
                .eq('id', directorateId)
                .single()
            setDirectorateName(dir?.name || null)

            // 2. Fetch existing report for the date
    ...
    useEffect(() => {
        ...
        // 1. Calculate SINE Total
        const sineIndicators = [
            "sine_atend_trabalhador", "sine_atend_trabalhador_online",
            "sine_atend_empregador", "sine_atend_empregador_online",
            "sine_seguro_desemprego", "sine_ctps_digital", "sine_vagas_captadas",
            "sine_ligacoes_recebidas", "sine_ligacoes_realizadas", "sine_curriculos",
            "sine_entrevistados", "sine_processo_seletivo", "sine_orientacao_profissional"
        ];
        const sineTotal = sineIndicators.reduce((acc, id) => {
            const val = data[id];
            return acc + (Number(val) || 0);
        }, 0);

        if (Number(data.sine_total) !== sineTotal) {
            newData.sine_total = String(sineTotal);
            hasChanges = true;
        }

        // 2. Calculate CP Totals per Center
        DIARIO_CENTROS.forEach(centro => {
            const cpIndicators = ["atendimento", "inscricoes", "pessoas_presentes", "ligacoes_recebidas", "ligacoes_realizadas"];
            const cpTotal = cpIndicators.reduce((acc, id) => {
                const val = data[`cp_${centro.id}_${id}`];
                return acc + (Number(val) || 0);
            }, 0);
            const totalFieldId = `cp_${centro.id}_total_procedimentos`;

            if (Number(data[totalFieldId]) !== cpTotal) {
                newData[totalFieldId] = String(cpTotal);
                hasChanges = true;
            }
        });
        ...
        <form onSubmit={handleSubmit} className="space-y-12 animate-in fade-in slide-in-from-bottom-2 duration-1000 pb-24">
            ...
            {fetching ? (
                <div className="space-y-12">
                    <div className="h-96 w-full bg-zinc-50 dark:bg-zinc-900/50 animate-pulse rounded-2xl border border-zinc-100 dark:border-zinc-800"></div>
                    <div className="h-96 w-full bg-zinc-50 dark:bg-zinc-900/50 animate-pulse rounded-2xl border border-zinc-100 dark:border-zinc-800"></div>
                </div>
            ) : (
                <>
                    {(() => {
                        const normalizedName = directorateName?.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "") || ""
                        const isSINE = normalizedName.includes('sine') || directorateId === 'd9f66b00-4782-4fc3-a064-04029529054b'
                        const isCP = normalizedName.includes('formacao') || normalizedName.includes('profissional') || normalizedName.includes('centro') || directorateId === 'd9f66b00-4782-4fc3-a064-04029529054b'

                        if (!isSINE && !isCP) {
                            return (
                                <Card className="p-12 border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/30 flex flex-col items-center justify-center text-center space-y-4 rounded-3xl">
                                    <div className="p-4 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800">
                                        <Loader className="w-8 h-8 text-blue-600 animate-spin-slow" />
                                    </div>
                                    <div className="space-y-2">
                                        <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100 italic">Módulo em Integração</h3>
                                        <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-sm">
                                            Os indicadores diários para <span className="font-bold text-blue-600">{directorateName}</span> estão sendo configurados.
                                            Em breve você poderá registrar os dados diários nesta área.
                                        </p>
                                    </div>
                                </Card>
                            )
