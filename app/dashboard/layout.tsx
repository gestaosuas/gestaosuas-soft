...
export default async function DashboardLayout({
    ...
    return (
        <div className="flex h-screen overflow-hidden bg-zinc-50/50 dark:bg-zinc-950 font-sans antialiased text-zinc-900 dark:text-zinc-50 selection:bg-primary/10 selection:text-primary flex-col md:flex-row">
            <MobileNav role={profile?.role as 'admin' | 'user'} directorates={profile?.role === 'admin' ? directorates || [] : profile?.directorates || []} />
            <Sidebar role={profile?.role as 'admin' | 'user'} directorates={profile?.role === 'admin' ? directorates || [] : profile?.directorates || []} />
