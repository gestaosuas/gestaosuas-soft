...
import { PopRuaDashboard } from "@/app/dashboard/graficos/pop-rua-dashboard"
import { FilePlus, Database, FileText, FolderOpen, CheckCircle2 } from "lucide-react"
import Link from "next/link"
import { Card, CardTitle } from "@/components/ui/card"

interface PopRuaPageClientProps {
    directorate: any
    submissions: any[]
    currentYear: number
...
export function PopRuaPageClient({
    ...
    const operations = [
        { 
            ...
            href: `/dashboard/relatorios/novo?setor=pop_rua&directorate_id=${directorate.id}`, 
            icon: FilePlus, 
            color: "text-indigo-600", 
            bg: "bg-indigo-50 dark:bg-indigo-900/20", 
            border: "hover:border-indigo-500",
            badge: latestMonth ? `Vigência: ${latestMonth}` : undefined
        },
        { label: "Ver Dados", href: `/dashboard/dados?setor=pop_rua&directorate_id=${directorate.id}`, icon: Database, color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20", border: "hover:border-emerald-500" },
        { label: "Relatório Mensal", href: `/dashboard/relatorios/mensal?setor=pop_rua&directorate_id=${directorate.id}`, icon: FileText, color: "text-violet-600", bg: "bg-violet-50 dark:bg-violet-900/20", border: "hover:border-violet-500" },
        { label: "Ver Relatórios", href: `/dashboard/relatorios/lista?setor=pop_rua&directorate_id=${directorate.id}`, icon: FolderOpen, color: "text-sky-600", bg: "bg-sky-50 dark:bg-sky-900/20", border: "hover:border-sky-500" },
    ]
                                ...
                                <Card className={`h-full min-h-[100px] bg-white dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 shadow-none ${item.border} transition-all duration-300 rounded-xl group-hover:shadow-lg flex flex-col items-center justify-center p-4 text-center relative overflow-hidden`}>
                                    ...
                                    {item.badge && (
                                        <div className="mt-2 flex items-center justify-center gap-1 px-1.5 py-0.5 bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/30 rounded-full w-fit relative z-10 mx-auto">
                                            ...
                                            <span className="text-[8px] font-black text-green-700 dark:text-green-400 uppercase tracking-tight">{item.badge}</span>
                                        </div>
                                    )}
                                </Card>
                            </Link>
